...
class Move {
    ...
    constructor(index: number, dx: number, dy: number) {
        this.index = index;
        this.dx = dx;
        this.dy = dy;
    }
...
function opposite(color: string): string {
...
class Checkers {
    ...
    selectedStone: Stone | null = null;
    currentTurn: string = white;
    
    setupInput(gameElement: HTMLElement) {
        gameElement.addEventListener("click", (e) => {
            if (this.currentTurn !== white) return;
    ...
    deepCopy(stones: Stone[]): Stone[] {
        return stones.map(s => new Stone(s.x, s.y, s.color));
    }

    moveStone(stones: Stone[], move: Move) {
    ...
    evaluateBoard(stones: Stone[], color: string) {
        let count = 0;
    ...
    isLegalMove(stones: Stone[], move: Move): boolean {
    ...
    getMoves(stones: Stone[], stone: Stone): Move[] {
        const moves: Move[] = [];
        const dx = [1, -1];
    ...
    // Returns [value, move]
    exploreMove(stones: Stone[], move: Move | null, color: string, depth: number): [number, Move | null] {
        ...
        for (let stone of stones)
            if (stone.color == color)
                for (let move of this.getMoves(stones, stone)) {
                    let [value, ] = this.exploreMove(
                        this.deepCopy(stones), move, 
                        opposite(color), 
                        depth + 1);
